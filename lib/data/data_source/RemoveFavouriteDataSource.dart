...
@Injectable(as: RemoveFavouriteDataSource)
class RemoveFavouriteDataSourceImpl implements RemoveFavouriteDataSource{
  ApiManger apiManger;
  RemoveFavouriteDataSourceImpl({required this.apiManger});

  Future<Either<Failers, RemoveFavouriteResponseDm>> RemoveFavourite(String productId)async{
   ...
   try{ if (connectivityResult.contains(ConnectivityResult.mobile) ||
      ...
      var response = await apiManger.RemoveFavourite(productId);
      var FavouriteResponse=RemoveFavouriteResponseDm.fromJson(response.data);
